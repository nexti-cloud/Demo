...
{
    public class ExerciseColumns : IExercise
    {
        ...
        public string Type { get; set; } = "columns";
        ...
        public List<ItemColumns> ItemList { get; set; }
        ...
        public ExerciseColumns(DataTable sheet, Section section, bool includeLocked)
        {
            ...
            //Nazvy sloupcu
            for (int i = 1; i < sheet.Rows[1].ItemArray.Length; i++)
            {
            ...
            for (int col = 0; col < ColumnList.Count; col++)
            {
                ...
                {
                    string word = sheet.Rows[row][col + 1] == DBNull.Value ? null : (sheet.Rows[row][col + 1]).ToString();
