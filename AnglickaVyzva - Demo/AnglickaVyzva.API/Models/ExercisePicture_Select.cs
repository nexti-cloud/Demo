...
{
    public class ExercisePicture_Select : IExercise
    {
        ...
        public string Type { get; set; } = "picture_select";
        public bool IsLock { get; set; }
        public bool IsDone { get; set; }

        public string Title { get; set; }
        public string Subtitle { get; set; }
        public List<ItemPicture_Select> ItemList { get; set; }
        

        public class ItemPicture_Select
        {
        ...
        public ExercisePicture_Select(DataTable sheet, Section section, bool includeLocked)
        {
            ...
            ItemList = new List<ItemPicture_Select>();
            ...
            for (int i = 2; i < sheet.Rows.Count; i++)
            {
                ...
                ItemPicture_Select item = new ItemPicture_Select();
                ...
                item.Translation = row.ItemArray[2] == DBNull.Value ? "" : Convert.ToString(row.ItemArray[2]).Trim();
                item.Question = row.ItemArray[3] == DBNull.Value ? "" : Convert.ToString(row.ItemArray[3]).Trim();

                for(int col = 4; col < row.ItemArray.Length; col++)
                {
                    if (row.ItemArray[col] == DBNull.Value)
                        break;

                    string image = section.DataFolderPath + (string)row.ItemArray[col];
                    image += ".png";
